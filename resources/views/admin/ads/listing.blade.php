...
var delID = 0;
var datatable_url = "{{url('admin/get_ads')}}";
...
function update_status(id,obj){
	...
	//$('#dvLoading').show();
	//var status=$(obj).attr("data-status-feature");
	//=0;
	//if(status==0){
		//feature_statuss=1;
	//}
	//else{
		feature_statuss=obj;
		//}
if(feature_statuss==0 || feature_statuss == ''){
	alert('Select Status First');
	return false;
	}
	 $.ajaxSetup({headers: {'X-CSRF-TOKEN': "{{ csrf_token() }}"}});
            $.ajax({
                url: "{{URL::to('/')}}/admin/feature_status/"+id,
                type: 'post',  // user.destroy
                data:{"csrf-token":"{{ csrf_token() }}","is_featured": feature_statuss},
