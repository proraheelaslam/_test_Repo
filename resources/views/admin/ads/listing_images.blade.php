					...
					if($images){
                        foreach($images as $single_page){?>
                            <tr>
                                <td><?=$i?></td>
                                <td>
                                 <img height="30" width="50" src="{{asset('public/uploads/ads/')}}/<?=$single_page['image']?>">
                                </td>
                                
                                <td>
                                     <a  <?=get_tooltip('Delete Image')?> href="javascript:void(0);" onclick="delete_record(<?=$single_page['id']?>)"><i class="fa fa-trash-o"></i></a>
...
$(document).ready(function () {
    ...
    $("#btn-delete").click(function(e) {
        if(delID != 0){
            ...
            $.ajax({
                url: "{{URL::to('/')}}/admin/ads/ad_image/"+delID,
