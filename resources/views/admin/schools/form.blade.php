...
<div class="form-group ">
    ...
    <input id="pac-input" value="<?=@$schools->address?>" name="address" class="form-control" type="text"
	    ...
	    {!! Form::text('website',null,['class'=>'form-control','required' => 'required']) !!}
                                ...
                                <label class=" control-label">Mission Statment</label>

                                    <textarea name="mission_statment" id="mission_statment" class="form-control" rows="4"><?=@$schools->mission_statment?></textarea>
                                ...
                                <label class=" control-label">Description</label>
                                    ...
                                    <textarea name="description" id="description" class="form-control" rows="4"><?=@$schools->description?></textarea>
 	...
 	<div class="form-group ">
	  ...
	  <select class="form-control m-bot15" name="is_active" id="is_active">
       <option <?php if (@$schools->is_active == '1') {?> selected <?php }?> value="1">Yes</option>
        <option <?php if (@$schools->is_active == '0') {?> selected <?php }?> value="0">No</option>
      </select>
	</div>
    
    <div class="form-group ">
      {!! Form::label('year_since','Year Since', array('class' => 'control-label')) !!}
        <input onKeyPress="if(this.value.length==4) return false;"  value="<?=@$schools->year_since?>" name="year_since" class="form-control" type="number"
        placeholder="Year Since" required>
	</div>
                            
                             <div class="form-group">
    
         {!! Form::label('image','School Logo', array('class' => 'control-label')) !!}
         
             <input type="file"  onchange="readImageURL(this);" onclick="BrowseFile()"  name="image" id="image" accept="image/*">
           @if ($errors->has('image'))
            <span  class="help-block">{{ $errors->first('image') }}</span>
            @endif
           <?php if(@$schools->image!=''){ ?>		
                <img src="{{asset('public/uploads/schools/'.@$schools->image)}}" width="70" height="70" id="new_image"  >
                
                 <a  onclick="delete_record(<?=@$schools->id?>)" href="javascript:;">Remove Image</a>
            <?php }
            else{?>
...
<div id="infowindow-content">
  ...
  <span id="place-address"></span>
          ...
          <input type="hidden" value="<?=@$user->latitude?>" id="lat" name="latitude"/>
<input type="hidden" value="<?=@$user->longitude?>" id="lng" name="longitude"/>                                                      


 <div class="form-group">
    {!! Form::submit(isset($schools) ? 'Update' : 'Save' ,['class'=>'btn btn-success pull-right']) !!}
</div>
...
<?php 
if(@$schools->latitude=='' && @$schools->longitude==''){
	 $lat=-33.8688;
	 $lng=151.2195;
	 }
	 else{
...
$(document).ready(function () {
    ...
    $("#btn-delete").click(function(e) {
        if(delID != 0){
            ...
            $.ajax({
                url: "{{URL::to('/')}}/admin/schools/remove_image_school",
