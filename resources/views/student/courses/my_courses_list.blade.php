<ul>
    @if(count($data['user_courses'])>0)
        @foreach($data['user_courses'] as $single_course)
            <li>
                <div class="stMc_myCourse_detail">
                    <div class="stMc_myCourse_img">
                        <a href="{{url('student/courses/course_video/').'/'.Hashids::encode($single_course->id)}}">
                            <img src="{{$single_course->full_image}}" alt="#"></a>
                    </div>
                    <div class="stMc_myCourse_right">
                        <div class="stMc_myCourse_text">
                            <strong>
                                <a href="{{url('student/courses/course_video/').'/'.Hashids::encode($single_course->id)}}">{{$single_course->name}}</a>
                            </strong>
