        ...
        <section class="cp_course_main cp_courseDetail_main">
            <div class="auto_content">
                <div class="cp_course_heading">
                    ...
                    @if($is_join_enabled==true)
                        @if(@Auth::guard('student')->user()->id==$courseDetail->student_id)
                            <a class="all_buttons crsDetail_joinNow_btn" target="_blank" href="https://elxdrive.com:3000/{{$courseDetail->course_key}}">{{Lang::get('label.Start Session')}}</a>
                        @else
                         <a class="all_buttons crsDetail_joinNow_btn" target="_blank" href="https://elxdrive.com:3000/{{$courseDetail->course_key}}">{{Lang::get('label.Join Now')}}</a>
                            ...
                            <div class="cp_course_left_inner">
                                <div class="cp_course_preview_main">
                                    <div class="cp_course_preview">
                                        ...
                                        @if(count($courseDetail->course_lectures)>0)
                                            @if($courseDetail->course_lectures[0]->file_type=='youtube')
                                                <iframe src="{{$courseDetail->course_lectures[0]->file_name}}" width="900" height="384" frameborder="0" allowfullscreen></iframe>
                                            @else
                                                <video controls>
                                                    <source src="{{$courseDetail->course_lectures[0]->full_file_path}}" type="video/mp4" />
                                                    {{Lang::get('label.Your browser does not support HTML5 video.')}}
                                                </video>
                                            @endif
                                            @elseif(!empty($firstLecturePreview->full_file_path))

                                            <video controls>
                                                <source src="{{$firstLecturePreview->full_file_path}}" type="video/mp4" />
                                                {{Lang::get('label.Your browser does not support HTML5 video.')}}
                                            </video>
                                            @else
                                            {{Lang::get('label.No Video Found')}}
                                        @endif
                                ...
                                <div class="cp_course_tabs">
                                    <div class="cp_course_tabs_title">
                                        <ul>
                                            ...
                                            <li><a href=".cp_course_tab3" onclick="increase_view('{{$courseDetail->student_id}}')">{{Lang::get('label.Instructor')}}</a></li>
                                                    ...
                                                    <div class="cp_instructor_inner">
                                                        ...
                                                        <div class="cp_instructor_detail">
                                                            ...
                                                            @if($courseDetail->instructor != null)
                                                                <p>{!! nl2br($courseDetail->instructor->about_student) !!} </p>
                                                            @endif
                                        ...
                                        <div class="cp_course_tags">
                                            ...
                                            <ul>
                                                @foreach($courseDetail->course_tags_arr as $row)
                                                    <li><span href="javascript:void(0);">{{$row}}</span></li>
                                                @endforeach
                                            </ul>
                                        </div>
                                    </div>
...
@section('javascript')
    ...
    <script>
        ...
        function increase_view(student_id) {
            $.ajaxSetup({headers: {'X-CSRF-TOKEN': "{{ csrf_token() }}"}});
            $.ajax({
                url: "{{ url('/courses/increase_views') }}",
                type: 'post',
                data:{"csrf-token":"{{ csrf_token() }}", "student_id" : student_id},
                success: function(result) {
                    console.log(result);
                }
            });
