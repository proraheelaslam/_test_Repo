...
class ForgotPasswordController extends Controller
{
    ...
    */
    ...
    public function forget_password(Request $request) {
        ...
        $rules['email'] = 'required|email|exists:schools,email';
        ...
        $instructor = Schools::where('email',$request->email)->where('is_active','1')->first();

        if($instructor){
            $rest_token='';
            $token =  str_random(30).$instructor->_id;
            $existToken  = DB::table('school_password_resets')->where('email',$request->email)->first();

            if($existToken){

                DB::table('school_password_resets')->where('email',$request->email)->update(array('token'=>$token));
                $rest_token = $token;
            ...
            $link = URL::to("/")."/school/password/reset/".$rest_token;
            ...
            return redirect('/school/password/reset');
