...
function wpbeginner_numeric_posts_nav() {
	...
	if ( get_previous_posts_link() )
		printf( '<li class="prevLink">%s</li>' . "\n", get_previous_posts_link('<span class="prev_pagi">Prev</span>') );
	...
	if ( get_next_posts_link() )
		printf( '<li class="nextLink">%s</li>' . "\n", get_next_posts_link('<span class="next_pagi">Next</span>') );
	echo '</ul></div>' . "\n";
}


//categories Single page fixed the pagination issue:
...
//pagination for tag page:
	// function tag_pagination_query( $query ) {
	// 	if ( $query->is_tag() && $query->is_main_query() ) {
	// 		$query->set( 'posts_per_page', 5 );
	// 	}
	// }
	// add_action( 'pre_get_posts', 'tag_pagination_query' );
...
function posts_link_next_class($format){
	$format = str_replace('href=', 'class="blog_next" href=', $format);
	return $format;
}
add_filter('next_post_link', 'posts_link_next_class');

function posts_link_prev_class($format) {
	$format = str_replace('href=', 'class="blog_prev" href=', $format);
	return $format;
}
add_filter('previous_post_link', 'posts_link_prev_class');

//Move WP_Comment field(textarea) to bottom of the other fields
function wpb_move_comment_field_to_bottom( $fields ) {
	...
	//http://wpsites.net/web-design/add-remove-change-order-of-comment-form-default-fields/
	...
	$arg['title_reply'] 	= __('Add Reviews & Rate');
	$arg['label_submit'] 	= __('Submit Review');
	return $arg;
	}
	add_filter('comment_form_defaults','isa_comment_reform');
	
	
	//change comment field text and field
	function wpsites_modify_comment_form_text_area($arg) {
		$arg['comment_field'] = '<p class="comment-form-comment"><label for="comment">' . _x( 'Review Comment', 'noun' ) . '</label><textarea id="comment" name="comment" cols="45" rows="1" aria-required="true"></textarea></p>';
		return $arg;
	}
	add_filter('comment_form_defaults', 'wpsites_modify_comment_form_text_area');
	
	
	//change text to reply on comment form
